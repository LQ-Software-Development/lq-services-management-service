...
export class CreateTaskService {
  ...
  async execute(createTaskDto: CreateTaskDto, projectId: string) {
    ...
    try {
      ...
      const lastIndex = await this.taskRepository.findOne({
      ...
      if (createTaskDto.approvalCriteria) {
        approvalCriteria = await Promise.all(
          createTaskDto.approvalCriteria.map(async (criterion) => {
            const newCriterion =
              this.approvalCriterionRepository.create(criterion);
      ...
      // Criar as atribuições se existirem
      if (createTaskDto.assignments?.length) {
        const assignments = createTaskDto.assignments.map((assignment) =>
