...
export class ListTasksService {
    ...
    async execute(filters: ListTasksFilterDto, organizationId: string) {
        ...
        if (filters.userId) {
            query.andWhere('assignments.userId = :userId', { userId: filters.userId });
        }
        ...
        if (filters.isActive !== undefined) {
            ...
            if (filters.isActive) {
                query.andWhere('task.startDate <= :today', { today })
                    .andWhere('task.endDate >= :today OR task.endDate IS NULL', { today });
            } else {
                query.andWhere('task.startDate > :today OR task.endDate < :today', { today });
            }
        }
